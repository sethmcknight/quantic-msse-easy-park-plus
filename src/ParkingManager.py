...
class CreateParkingLotCommand:
    ...
    def execute(self) -> bool:
        self._previous_levels = self._parking_lot.levels.copy() if self._parking_lot.levels else None
...
class ParkingLot:
    ...
    def get_levels_for_lot(self, lot_name: str) -> List[int]:
        ...
        return [level.get_level() for level in self.levels if level.get_name() == lot_name]
    
    def set_name(self, name: str) -> None:
        """Set the name of the parking lot"""
        if self.levels:
            self.levels[0].set_name(name)
    
    def get_slot_by_registration(self, reg: str) -> Optional[int]:
        """Get slot number by registration number"""
        return self._find_slot(lambda v: v.registration_number == reg)
    
    def get_slots_by_color(self, color: str) -> List[int]:
        """Get slot numbers by vehicle color"""
        return self._find_slots(lambda v: v.color.lower() == color.lower())
    
    def get_slots_by_make(self, make: str) -> List[int]:
        """Get slot numbers by vehicle make"""
        return self._find_slots(lambda v: v.make.lower() == make.lower())
    
    def get_slots_by_model(self, model: str) -> List[int]:
        """Get slot numbers by vehicle model"""
        return self._find_slots(lambda v: v.model.lower() == model.lower())
    
    def get_vehicle(self, slot_number: int) -> Optional[Vehicle.Vehicle]:
        """Get vehicle by slot number"""
        for level in self.levels:
            slots = level.get_slots()
            if 0 <= slot_number - 1 < len(slots):
                return slots[slot_number - 1].get_vehicle()
        return None
    
    def _create_vehicle(self, reg: str, make: str, model: str, color: str, is_electric: bool, is_motorcycle: bool) -> Vehicle.Vehicle:
    ...
    def leave(self, slot_number: int) -> bool:
        ...
        for level in self.levels:
            slots = level.get_slots()
            if 0 <= slot_number - 1 < len(slots):
                vehicle = slots[slot_number - 1].leave()
                if vehicle:
